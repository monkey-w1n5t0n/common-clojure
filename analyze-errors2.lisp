;;;; Improved Error Analysis Script
;;;; Usage: sbcl --noinform --disable-debugger --non-interactive --load analyze-errors2.lisp
...
(defun classify-error (error-type error-msg)
  "Classify the error with accurate root cause and difficulty."
  (let ((msg error-msg))
    (cond
      ;; Reader errors
      ...
      ;; Undefined symbol - MOST COMMON pattern
      ((and (search "Undefined symbol" msg)
            (search "Undefined symbol: " msg))
       (let* ((start (+ (search "Undefined symbol: " msg) 19))
              (rest (subseq msg start))
              ;; The symbol name ends at end of first line
              (end (or (position #\Newline rest) (length rest)))
              (sym (subseq rest 0 end)))
         (list :root-cause (format nil "Missing core function/symbol: ~A" sym)
      ...
      ((search "Unknown special form" msg)
       (let* ((start (search "form: " msg))
         ...
         (list :root-cause (format nil "Missing special form: ~A" form-name)
               :fix (format nil "Implement ~A special form in clojure-eval" form-name)
               :difficulty :MEDIUM
               :specific form-name)))
      ...
      ((search "Unknown function" msg)
       (let* ((start (search "function: " msg))
              (fn-name (if start
                          (let* ((rest (subseq msg (+ start 10)))
                                 (end (or (position #\) rest) (position #\Space rest) (length rest))))
                            (subseq rest 0 end))
                          "unknown")))
         (list :root-cause (format nil "Missing core function: ~A" fn-name)
               :fix (format nil "Register '~A' in setup-core-functions" fn-name)
               :difficulty :EASY
               :specific fn-name)))

      ;; Cannot apply non-function
      ((search "Cannot apply non-function" msg)
       (list :root-cause "Calling a non-function value"
             :fix "Register the function or fix binding"
             :difficulty :MEDIUM
             :specific "apply"))
      ...
      ;; Throwable / Java class references
      ((or (search "Throwable" msg) (search "just testing" msg))
       (list :root-cause "Java class reference (Throwable, Exception etc.)"
             :fix "Add stubs for Java exception classes"
             :difficulty :MEDIUM
             :specific "java-stubs"))

      ;; HASH-MAP as function call
      ((search "HASH-MAP" msg)
       (list :root-cause "HASH-MAP symbol resolution issue"
             :fix "Fix symbol resolution for hash-map"
             :difficulty :EASY
               :specific "hash-map"))
...
(let ((results nil)
  ...
  (with-open-file (out output-path :direction :output :if-exists :supersede)
    ;; Detailed results
    (format out "=== DETAILED ERROR ANALYSIS FOR 22 TEST FILES ===~%~%")
    ...
    (dolist (result results)
      (let ((name (getf result :name))
        ...
        (when (eq status :failed)
          ...
          (when first-form
            (format out "  Failing form: ~A~%" (truncate-str first-form 150)))
    ...
    ;; Summary table sorted by difficulty then alphabetically
    (let ((sorted
