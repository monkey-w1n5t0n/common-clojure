...
# Write function definitions to a temp file (no execution)
cat > /tmp/clj-analysis-fns.lisp << 'ENDOFCODE'
(defun diagnose-one-file (name)
  (let ((path (make-pathname :name name :type "clj" :defaults cl-clojure-eval::*test-dir*)))
...
;; Set up test dir
(setf cl-clojure-eval::*test-dir*
  (make-pathname :directory (list :absolute "home" "w1n5t0n" "src" "common-clojure" "clojure-tests")))

ENDOFCODE

# Write the execution part to a separate file
cat > /tmp/clj-analysis-run.lisp << 'ENDOFCODE'
...
sbcl --noinform --disable-debugger --non-interactive \
  ...
  --load /tmp/clj-analysis-fns.lisp \
  --load /tmp/clj-analysis-run.lisp \
  2>/dev/null
